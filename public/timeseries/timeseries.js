...
(function() {
    ...
    var timeseries = function(spaced, data, enableBrush, options = { startHour: 0, endHour: 24 }) {
        classd = spaced.replace(new RegExp(" "), ".");
        render(classd, spaced, data, enableBrush, options);
    }
    ...
    */
    ...
    function render(classd, spaced, data, enableBrush, options) {
        ...
        if (lessThanDay(padding.pad)) {
            ...
            xFormat = "%m/%d/%y";
            yFormat = "%H:%M";
            var start = new Date(2012, 0, 1, options.startHour, 0, 0, 0).getTime();
            var stop = new Date(2012, 0, 1, options.endHour - 1, 59, 59, 59).getTime();
